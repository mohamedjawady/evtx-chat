...
def ask_ollama(question, context, techniques_used=None):
    ...
    IMPORTANT: Do not return markdown code blocks containing JSON, as this will cause rendering errors.
    Your response should be plain text formatted for human reading.
    
    Answer:"""

    # Add information about techniques used if available
    techniques_info = ""
    if techniques_used and len(techniques_used) > 0:
        techniques_info = "I used the following retrieval techniques to find this information: " + ", ".join(techniques_used)
    ...
    try:
        ...
        # Clean any accidental JSON code blocks from the response
        response = response.replace("```json", "").replace("```", "")
        
        return response
    except Exception as e:
        logging.error(f"Error during Ollama inference: {str(e)}")
        return f"Error generating response: {str(e)}. Please make sure Ollama is running and the deepseek-r1:7b model is available."